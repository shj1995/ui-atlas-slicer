...
启动后不会自动加载任何素材。点击左侧“替换图片”，或把 PNG 拖入素材卡片/画布，也可以直接粘贴剪贴板中的图片。

![空画布状态](docs/images/usage/01-empty-state.png)

### 2. 自动识别并接受结果

点击左侧“自动识别”，完成扫描后点击右侧“接受结果并转为切片”。

![自动识别结果](docs/images/usage/04-detection-results.png)

### 3. 导出 ZIP

点击底部“导出 ZIP”，将当前切片一次性打包下载。

![导出 ZIP](docs/images/usage/02-image-imported.png)
...
感谢 [LINUX DO](https://linux.do/) 对开源社区的支持。
